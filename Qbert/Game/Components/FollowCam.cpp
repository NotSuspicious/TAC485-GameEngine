      ...
      , m_game(pGame)
{
    ...
    //mrwTODO this 90 rotation accounts for the orientation of the model :(
}
...
void FollowCam::Update(float deltaTime)
{
    ...
    Matrix4 mat = Matrix4::CreateRotationY(m_elevation)
                  ...
                  * Matrix4::CreateTranslation(m_actor->m_constants.modelToWorld.GetTranslation() + mCenter + p);
    mat.Invert();
    pCamera->SetViewMatrix(mat);
}