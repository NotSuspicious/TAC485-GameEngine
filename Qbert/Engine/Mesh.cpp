...
Mesh::Mesh(Renderer *renderer, VertexBuffer *vertexBuffer, Material *material)
    : m_renderer(renderer),
...
Mesh::~Mesh()
{
    ...
    // the material is not unique to the mesh, so we do not delete it here
}
...
bool Mesh::Load(void *vertexData, uint32_t vertexDataSize, void *indexData, uint32_t numIndex, uint32_t indexStride,
                Material *material)
{
    m_vertexBuffer = new VertexBuffer(m_renderer, vertexData, vertexDataSize, indexData, numIndex, indexStride);
    m_material = material;
...
{
    Mesh * pMesh = new Mesh(Renderer::Get(), nullptr, nullptr);
    if (!pMesh->Load(fileName, pAssetManager))
    {
        delete pMesh;
        return nullptr;
    }
...
void Mesh::Draw(SDL_GPUCommandBuffer *commandBuffer, SDL_GPURenderPass *renderPass) const
{
