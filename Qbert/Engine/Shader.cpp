...
/// Call this function AFTER you've set any optional settings (including SetDepthFlags), but before you call SetActive
...
bool Shader::CreatePipeline(const SDL_GPUVertexAttribute *vertexAttributes, uint32_t numVertexAttributes,
                            uint32_t vertexStride)
{
    ...
    blendState.enable_blend = m_blendEnabled;
    blendState.color_blend_op = SDL_GPU_BLENDOP_ADD;
    blendState.src_color_blendfactor = m_srcColorBlendFactor;
    blendState.dst_color_blendfactor = m_dstColorBlendFactor;
    // Use the same settings for alpha as for color
    blendState.alpha_blend_op = SDL_GPU_BLENDOP_ADD;
    blendState.src_alpha_blendfactor = m_srcColorBlendFactor;
    blendState.dst_alpha_blendfactor = m_dstColorBlendFactor;
    // We aren't using the color mask, so leave enable_color_write_mask false (default)
    ...
    bool hasDepthTarget = (m_depthFlags & (DepthFlags::TEST | DepthFlags::WRITE)) != DepthFlags::NONE;
    pipelineInfo.depth_stencil_state.enable_depth_test = (m_depthFlags & DepthFlags::TEST) != DepthFlags::NONE;
    pipelineInfo.depth_stencil_state.enable_depth_write = (m_depthFlags & DepthFlags::WRITE) != DepthFlags::NONE;
    pipelineInfo.target_info.color_target_descriptions = &colorTargetDesc;
    pipelineInfo.target_info.num_color_targets = 1;
    pipelineInfo.target_info.depth_stencil_format = m_renderer->GetTextureFormat();
    pipelineInfo.target_info.has_depth_stencil_target = hasDepthTarget;
