...
namespace Math
{
    ...
    {
        return a + (b - a) * (static_cast<float>(rand()) / (static_cast<float>(RAND_MAX) + 1.0f));
    }
...
public:
    ...
    {
        return Vector2(b.x + a.x * (1 - f), b.y + a.y * (1 - f));
    }
...
public:
    ...
    {
        return Math::IsZero(a.x - b.x) && Math::IsZero(a.y - b.y) && Math::IsZero(a.z - b.z);
    }
...
{
    const float len = 1.0f / vec.Length();
...
public:
    ...
    // Angles are in radians. Order matches Matrix4::CreateYawPitchRoll: yaw (Y), pitch (X), roll (Z)
    static Quaternion CreateFromYawPitchRoll(float yaw, float pitch, float roll);

    // Create a quaternion from Euler angles provided as a Vector3.
    // Interpretation: Vector3.x = pitch, Vector3.y = yaw, Vector3.z = roll (radians)
    // This maps to CreateFromYawPitchRoll(yaw, pitch, roll) internally.
...
{
    // Build quaternions for each axis (angles are in radians)
    Quaternion qYaw(Vector3::UnitY, Math::ToRadians(yaw));
    Quaternion qPitch(Vector3::UnitX, Math::ToRadians(pitch));
    Quaternion qRoll(Vector3::UnitZ, Math::ToRadians(roll));

    // The matrix order is Rz(roll) * Rx(pitch) * Ry(yaw)
    // That means we apply yaw first, then pitch, then roll.
    Quaternion temp = Quaternion::Concatenate(qYaw, qPitch); // yaw then pitch
    Quaternion result = Quaternion::Concatenate(temp, qRoll); // (yaw then pitch) then roll
    result.Normalize();
    return result;
}
