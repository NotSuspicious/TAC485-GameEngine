...
bool Vector2::UnitTest()
{
    ...
    // normalize
    {
        ...
        testNorm = Vector2::Normalize(Vector2(10.0f, -10.0f));
    ...
    // lerp
    {
        Vector2 testLerp(-1.0f, 0.0f);

        Vector2 lerp = Lerp(testLerp, Vector2::Zero, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.x, testLerp.x);
        isOk &= Math::IsCloseEnuf(lerp.y, testLerp.y);
        lerp = Lerp(testLerp, Vector2::Zero, 1.0f);
        isOk &= Math::IsCloseEnuf(lerp.x, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.y, 0.0f);
        lerp = Lerp(testLerp, Vector2::Zero, 0.5f);
        isOk &= Math::IsCloseEnuf(lerp.x, -0.5f);
        isOk &= Math::IsCloseEnuf(lerp.y, 0.0f);
        lerp = Lerp(testLerp, Vector2::Zero, 2.0f);
...
static bool UnitTest_Vector3()
{
    ...
    // normalize
    {
        ...
        testNorm = Vector3::Normalize(Vector3(0.0f, -10.0f, 10.0f));
    ...
    // lerp
    {
        Vector3 testLerp(-1.0f, 0.0f, 3.0f);

        Vector3 lerp = Vector3::Lerp(testLerp, Vector3::Zero, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.x, testLerp.x);
        isOk &= Math::IsCloseEnuf(lerp.y, testLerp.y);
        isOk &= Math::IsCloseEnuf(lerp.z, testLerp.z);
        lerp = Vector3::Lerp(testLerp, Vector3::Zero, 1.0f);
        isOk &= Math::IsCloseEnuf(lerp.x, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.y, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.z, 0.0f);
        lerp = Vector3::Lerp(testLerp, Vector3::Zero, 0.5f);
        isOk &= Math::IsCloseEnuf(lerp.x, -0.5f);
        isOk &= Math::IsCloseEnuf(lerp.y, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.z, 1.5f);
        lerp = Vector3::Lerp(testLerp, Vector3::Zero, 2.0f);
        isOk &= Math::IsCloseEnuf(lerp.x, 1.0f);
        isOk &= Math::IsCloseEnuf(lerp.y, 0.0f);
    ...
    {
        Vector3 chainVector = 2.0f * testVector - Vector3::Cross(Vector3(1.0f, 0.0f, 0.0f), Vector3(0.0f, 1.0f, 0.0f)) *
                              Vector3::Dot(Vector3(0.0f, 0.0f, 1.0f), Vector3(0.0f, 0.0f, 2.0f));
...
static bool UnitTest_Vector4()
{
    ...
    // normalize
    {
        ...
        testNorm = Vector4::Normalize(Vector4(0.0f, -10.0f, 10.0f, 0.0f));
        isOk &= Math::IsCloseEnuf(testNorm.x, 0.0f);
        isOk &= Math::IsCloseEnuf(testNorm.y, -0.70710678f);
        isOk &= Math::IsCloseEnuf(testNorm.z, 0.70710678f);
    ...
    // lerp
    {
        Vector4 lerp = Vector4::Lerp(testVector, Vector4::Zero, 0.0f);
        ...
        lerp = Vector4::Lerp(testVector, Vector4::Zero, 1.0f);
        isOk &= Math::IsCloseEnuf(lerp.x, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.y, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.z, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.w, 0.0f);
        lerp = Vector4::Lerp(testVector, Vector4::Zero, 0.5f);
        isOk &= Math::IsCloseEnuf(lerp.x, -0.5f);
        isOk &= Math::IsCloseEnuf(lerp.y, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.z, 1.5f);
        isOk &= Math::IsCloseEnuf(lerp.w, 0.5f);
        lerp = Vector4::Lerp(testVector, Vector4::Zero, 2.0f);
        isOk &= Math::IsCloseEnuf(lerp.x, 1.0f);
        isOk &= Math::IsCloseEnuf(lerp.y, 0.0f);
        isOk &= Math::IsCloseEnuf(lerp.z, -3.0f);
...
static bool UnitTest_Matrix4()
{
    ...
    //Transform a vector position
    {
        ...
        Vector3 testPos = Matrix4::Transform(pos, testMat);
    ...
    //Transform a vector direction
    {
        ...
        Vector3 testDir = Matrix4::Transform(dir, testMat, 0.0f);
    ...
    //create a lookAt
    {
        ...
        Vector3 fwd = Matrix4::Transform(Vector3(0.0f, 0.0f, 1.0f), testLook, 0.0f);
        isOk &= Math::IsCloseEnuf(fwd.x, 1.0f);
        isOk &= Math::IsCloseEnuf(fwd.y, 0.0f);
        isOk &= Math::IsCloseEnuf(fwd.z, 0.0f);
        Vector3 up = Matrix4::Transform(Vector3(0.0f, 1.0f, 0.0f), testLook, 0.0f);
        isOk &= Math::IsCloseEnuf(up.x, 0.0f);
        isOk &= Math::IsCloseEnuf(up.y, 1.0f);
        isOk &= Math::IsCloseEnuf(up.z, 0.0f);
        Vector3 left = Matrix4::Transform(Vector3(1.0f, 0.0f, 0.0f), testLook, 0.0f);
...
/*static*/
bool Vector3::UnitTest()
{
    return UnitTest_Vector3();
}

/*static*/
bool Vector4::UnitTest()
{
    return UnitTest_Vector4();
}

/*static*/
bool Matrix4::UnitTest()
{
    return UnitTest_Matrix4();
}

/*static*/
bool Matrix3::UnitTest()
{
    bool isOk = true;
